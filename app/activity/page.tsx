...
function getActivityIcon(type: Activity['type']) {
  switch (type) {
    case 'uploaded':
      return <Upload className="h-4 w-4 text-vault-accent" />
    case 'deleted':
      return <Trash2 className="h-4 w-4 text-destructive" />
    case 'restored':
      return <RefreshCw className="h-4 w-4 text-vault-success" />
    default:
      ...
      <div className="flex min-h-screen items-center justify-center bg-[#0A0A0A] px-4">
        <Card className="w-full max-w-md border-vault-border bg-vault-surface">
          <CardContent className="p-6">
            <p className="text-vault-text">Please sign in to view your activity.</p>
          </CardContent>
        </Card>
      </div>
    )
    ...
    <div className="pl-60 pt-16 min-h-screen bg-[#0A0A0A]">
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-vault-text">Activity</h1>
            <p className="text-sm text-vault-text-muted mt-1">
              Recent uploads, deletes, and restores
            </p>
          </div>
          <Button
            variant="outline"
        ...
        {isLoading ? (
            ...
            <CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="max-w-md text-sm text-vault-text-muted">
                  Your recent uploads, deletes, and restores will appear here.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-3">
            {activities.map((activity) => (
              <Card key={`${activity.backupId}-${activity.timestamp}`} className="border-vault-border bg-vault-surface">
                <CardContent className="flex items-center gap-4 p-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-vault-bg">
                    {getActivityIcon(activity.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-vault-text">{activity.filename}</span>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-vault-bg text-vault-text-muted">
