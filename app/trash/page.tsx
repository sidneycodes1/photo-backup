    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0A0A0A] px-4">
        <Card className="w-full max-w-md border-vault-border bg-vault-surface">
          <CardHeader>
            <CardTitle className="text-vault-text">Authentication Required</CardTitle>
            <CardDescription className="text-vault-text-muted">Please sign in to access your trash.</CardDescription>
          </CardHeader>
        </Card>
      </div>
    )
    ...
    <div className="pl-60 pt-16 min-h-screen bg-[#0A0A0A]">
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-vault-text">Trash</h1>
        ...
        {isLoading ? (
            ...
            <CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="max-w-md text-sm text-vault-text-muted">
                  Items you delete will appear here for 30 days before permanent deletion.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            {backups.map((backup) => {
              ...
              return (
                <Card key={backup.id} className="border-vault-border bg-vault-surface">
                  <CardContent className="flex items-center gap-4 p-4">
                    <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-vault-bg overflow-hidden">
                      {backup.thumbnailCid ? (
                        <img
                          src={`https://gateway.lighthouse.storage/ipfs/${backup.thumbnailCid}`}
                          alt={backup.originalFilename ?? 'Thumbnail'}
                          className="h-full w-full object-cover"
                        />
                      ) : (
                        <div className="flex h-full w-full items-center justify-center text-vault-text-muted text-xs">
                          {backup.mimeType?.startsWith('video/') ? 'Video' : 'Image'}
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-vault-text-muted">
                        ...
                        <span>•</span>
                        <span>{formatDate(backup.deletedAt ?? backup.createdAt)}</span>
                      </div>
                      <div className="flex items-center gap-2 mt-2 text-xs">
                        <Clock className="h-3 w-3 text-vault-accent" />
                        <span className={daysLeft <= 7 ? 'text-destructive' : 'text-vault-text-muted'}>
      ...
      <ConfirmDialog
        ...
        description="This will restore the item to your main vault."
      ...
      <ConfirmDialog
        ...
        description="This action cannot be undone. The encrypted file will remain on decentralized storage, but all metadata will be removed."
